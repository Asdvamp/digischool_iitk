...
def testPage(request):
    ...
    if active_status:
        extract_user__user_signup_database = login_models.USER_SIGNUP_DATABASE.objects.filter(id=user_id)[
            0]
        ...
        if extract_user__user_signup_database.user_category == "STUDENT":
            ...
            all_course_id = course_models.CLASS_COURSES_MAPPING.objects.filter(
                unique_id=generated_unique_id)
            all_course_id = all_course_id.strip().split(" ")

            return render(request, "student_test.html", {"csrf_token": csrf_token, "user_courses": course_models.AVAILABLE_COURSE, "all_course_list": all_course_id})
    else:
        # session is inactive.
...
def testUploaded(request):
    ...
    if active_status and extract_user__user_signup_database.user_category == "TEACHER":
        ...
        start_datetime = datetime(
            s_date[0], s_date[1], s_date[2], s_time[0], s_time[1], 0, 0)

        e_date, e_time = test_end_date_date.split(
            "-"), test_end_time_time.split(":")
        end_datetime = datetime(
            e_date[0], e_date[1], e_date[2], e_time[0], e_time[1], 0, 0)
        ...
        try:
            setting_test = test_models.ALL_TESTS(test_title=test_title, test_instruction=test_instruction, start_datetime=start_datetime,
                                                 end_datetime=end_datetime, files=test_files, test_unique_id=test_unique_id, course_mapping=course_in_context)
            setting_test.save()
        except:
