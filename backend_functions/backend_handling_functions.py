...
def auto_assign_course(user_cls, user_sec, user_category):
    if user_category == "STUDENT":
        ...
        class_course_mapping_entry = course_models.CLASS_COURSES_MAPPING.objects.filter(
...
def course_instructor_assigning(teacher_entry):
    ...
    course_unique_id = subject_code + teacher_entry.user_class + \
        teacher_entry.user_section + str(OFFERING_YEAR)
    selected_course = course_models.AVAILABLE_COURSE.objects.filter(
        course_id=course_unique_id)[0]
